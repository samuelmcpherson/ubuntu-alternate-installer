...
chroot $TEMPMOUNT /bin/bash -c "apt-get install -y kwrite akregator marble kdenlive kigo kompare krfb ktimer kmousetool node-typescript && echo '---> apt install kwrite akregator marble kdenlive kigo kompare korganizer krfb ktimer kmousetool node-typescript succeeded <--------------------------------------------------------------' || { echo 'apt install kwrite akregator kalarm kapman kblocks kbreakout marble kdenlive kigo kompare korganizer krfb ktimer kmousetool node-typescript failed'; exit 1; }" || exit 1
